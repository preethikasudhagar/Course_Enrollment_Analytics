...
async def seed_all_data(db: AsyncSession):
    ...
    for c, target_count in courses:
        await db.refresh(c)
        # Pick random students (up to target_count)
        pool = list(students)
        random.shuffle(pool)
        count = 0
        for s in pool:
            if count >= target_count: break
            # Specific dates for monthly trends (Jan-Apr)
        ...
        # Add Analytics for the course
        trend_msg = f"Growth index: {random.uniform(1.1, 1.8):.2f}"
        score = (count / c.seat_limit) * 10 if c.seat_limit > 0 else 0
        # Historical enrollment sample (Jan-Apr)
