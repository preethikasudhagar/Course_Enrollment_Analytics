...
const StatusBadge = ({ enrolled, limit, className }) => {
    ...
    if (remainingSeats <= 5) status = { text: 'High Demand', color: 'bg-amber-100 text-amber-700 border-amber-200' };
    return (
        <div className={twMerge("flex flex-col gap-2", className)}>
            <div className="flex items-center justify-between gap-4">
                <span className={clsx(
                    ...
                    <span className={clsx("w-1.5 h-1.5 rounded-full", status.text === 'High Demand' ? 'bg-amber-500' : 'bg-emerald-500')}></span>
                    {status.text}
                </span>
